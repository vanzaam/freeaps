...
def find_file_ref(project, name)
  project.files.find { |f| (f.display_name == name) || (f.path == name) } ||
    project.objects.select { |o| o.isa == 'PBXFileReference' }.find { |f| f.path == name || f.name == name }
end
...
FRAMEWORKS.each do |fw_name|
  ...
  unless file_ref
    puts "⚠️ Missing file ref: #{fw_name}. Build the dependency once or open workspace to materialize products."
    next
  end
